set "pathTo=build/apps/Console/RelWithDebInfo/"

REM Transfert existing dataset
REM "%pathTo%\Console.exe" --mode generate --size 1 --ai="MCTS_Deterministic(20000;4)" --ai="MCTS_Deterministic(10000;8)" --in pureMC1000 --out mixed

set /a i=0

:loop
set /a i+=1
echo Iteration %i%
"%pathTo%\Console.exe" --mode generate --size 160 --ai="MCTS_Deterministic(40000;4)" --ai="MCTS_Deterministic(20000;8)" --ai="MCTS_Deterministic(40000;4)" --ai="MCTS_Deterministic(20000;8)" --in mixed --out mixed --threads 8
goto loop
